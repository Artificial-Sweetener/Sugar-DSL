...
"""Add or update GPLv3-or-later license headers in tracked source files."""
...
PROJECT_LINE = "Compose human-readable ComfyUI workflows with SugarCubes"
...
def _license_body(prefix: str) -> str:
    """Return the GPLv3 notice body using the requested comment prefix."""

    return "\n".join(
        (
            f"{prefix}",
            f"{prefix}    This program is free software: you can redistribute it and/or modify",
            f"{prefix}    it under the terms of the GNU General Public License as published by",
            ...
            f"{prefix}    GNU General Public License for more details.",
            f"{prefix}",
            f"{prefix}    You should have received a copy of the GNU General Public License",
...
def _comment_prefix(path: Path) -> str:
    ...
    if path.suffix not in SUPPORTED_SUFFIXES:
        raise ValueError(f"Unsupported source file suffix for license header: {path}")
    return "#"
...
def update_header(path: Path) -> bool:
    ...
    if "GNU General Public License" in normalized_content[:1500]:
        print(f"Skipping {path}: unknown GPL header already present")
        return False
